...
`ObjectId` сгенерированны MongoDB для поля `_id` подставляется в селектор следующим образом:
